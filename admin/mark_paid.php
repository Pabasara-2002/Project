...
if (isset($_GET['id'])) {
    ...
    if ($order && strtolower(trim($order['order_status'])) === 'approved')
 {
        ...
        header("Location: view_orders.php?msg=Order marked as paid");
        exit;
    } else {
        header("Location: view_orders.php?msg=Only approved orders can be marked as paid");
        exit;
    }
} else {
    header("Location: view_orders.php?msg=Invalid request");
    exit;
}
?>
